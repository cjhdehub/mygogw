...
func (c *httpClient) QueryExt(command,db string) (error, interface{}) {
	req, err := c.makeRequest(queryURL(c.url, command, db), bytes.NewReader([]byte("")))
	if err != nil {
		return err,nil
	}
...
func (c *httpClient) Query(command string) error {
	...
	err , _ = c.doRequest(req, http.StatusOK)
	return err
}
...
func (c *httpClient) WriteStream(r io.Reader) error {
	...
	err , _ = c.doRequest(req, http.StatusNoContent)
	return err
}
...
func (c *httpClient) doRequest(
	...
	if err != nil {
		return fmt.Errorf("Fatal error reading body: %s", err),nil
	}
...
func queryURL(u *url.URL, command string,db ... string) string {
	params := url.Values{}
