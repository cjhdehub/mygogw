...
func (ssl *streamSocketListener) listen() {
	...
	for {
		...
		glog.V(10).Info("ssl.connectionsMtx.Lock 0")

		ssl.connectionsMtx.Lock()
		glog.V(10).Info("ssl.connectionsMtx.Lock 1")

		if ssl.MaxConnections > 0 && len(ssl.connections) >= ssl.MaxConnections {
			glog.V(10).Info("ssl.MaxConnections exceed max")
...
func (ssl *streamSocketListener) read(c net.Conn) {
	...
	for {
		...
		glog.V(10).Info("scnr.Scan 2")

		for index, m := range metrics {
			glog.V(10).Info(index,"AddFields start")
			ssl.AddFields(m.Name(), m.Fields(), m.Tags(), m.Time())
			glog.V(10).Info(index,"AddFields over")

		}
		glog.V(10).Info("scnr.Scan 3")

	}

	if err := scnr.Err(); err != nil {
		glog.Error("streamSocketListener read",err)
		if err2, ok := err.(net.Error); ok && err2.Timeout() {
			log.Printf("D! Timeout in plugin [input.socket_listener]: %s", err2)
...
func (sl *SocketListener) Start(acc telegraf.Accumulator) error {
	...
	case "tls":
		...
		listener, err := tls.Listen("tcp", spl[1], &config)
		if err != nil {
