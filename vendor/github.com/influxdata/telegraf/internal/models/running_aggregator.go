...
func (r *RunningAggregator) Add(in telegraf.Metric) bool {
    ...
    glog.V(10).Info("RunningAggregator add ",len(r.metrics))
	r.metrics <- in
...
func (r *RunningAggregator) push(acc telegraf.Accumulator,period time.Duration) {
	r.a.Push(acc,period)
}
...
func (r *RunningAggregator) Run(
	...
	for {
		...
		case m := <-r.metrics:
			...
			/*log.Println("new metrics",r.Config.Period.String())
			log.Println("must be in end time",r.periodEnd.Add(truncation).Add(r.Config.Delay).String())
			log.Println("current time",m.Time().String())*/
			...
			r.add(m)
		/*case <-periodT.C:
			log.Println("periodT.C push0")
			*//*log.Println("new push",r.Config.Period.String())
			log.Println("periodStart=",r.periodStart.String())
			log.Println("periodEnd=",r.periodEnd.String())*//*

			r.periodStart = r.periodEnd
			r.periodEnd = r.periodStart.Add(r.Config.Period)
			r.push(acc,r.Config.Period)
			r.reset()
			log.Println("periodT.C push1")*/
		}
	}
}
