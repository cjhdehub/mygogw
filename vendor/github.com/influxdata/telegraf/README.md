...
./telegraf --config ./telegraf.conf  --input-filter http_listener

curl -i -XPOST 'http://localhost:8186/write?db=network_quality' --data-binary

## 每秒一个Ping,ttl=500
'ping,srcIp=1.1.1.1,dstIp=1.2.2.2 ttl=500'

## 每秒一个Ping,icmp不返回
'ping,srcIp=1.1.1.1,dstIp=1.2.2.2 ttl=0'

## 触发告警
* 'ping,srcIp=1.1.1.1,dstIp=1.2.2.2 jumps="{\"route\":[{\"seq\":1,\"ttl\":20,\"dstIp\":\"2.2.2.5\"}]}",ttl=2234,lostRate=12.4,score=10'
* 'ping,srcIp=1.1.1.1,dstIp=1.2.2.2 failureDuration=12'
## 每60秒,如果不触发,不用发送数据


某条路径上的首次告警metric才会写入influxdb,该路径上的连续告警metric不会覆盖首条数据

'pingNode2City,srcIp=1.1.1.1 srcCity=杭州,dstCity=北京,avlPathCntRate=3/4,ttl=500,lostRate=98,score=80,threshold=30,alertGroup="group1,group2",onAlert=1'




DROP CONTINUOUS QUERY "1440m" on network_quality
DROP CONTINUOUS QUERY "30m" on network_quality
DROP CONTINUOUS QUERY "5m" on network_quality
DROP CONTINUOUS QUERY "120m" on network_quality



CREATE CONTINUOUS QUERY "5m" ON network_quality BEGIN SELECT  mean(lost) as lost,mean(mdev) as mdev,mean(score) as score,mean(ttl) as ttl INTO network_quality.autogen.m5ping FROM network_quality.autogen.ping GROUP BY srcIp,dstIp,time(5m) END

CREATE CONTINUOUS QUERY "30m" ON network_quality BEGIN SELECT  mean(lost) as lost,mean(mdev) as mdev,mean(score) as score,mean(ttl) as ttl INTO network_quality.autogen.m30ping FROM network_quality.autogen.ping GROUP BY srcIp,dstIp,time(30m) END


CREATE CONTINUOUS QUERY "120m" ON network_quality BEGIN SELECT  mean(lost) as lost,mean(mdev) as mdev,mean(score) as score,mean(ttl) as ttl INTO network_quality.autogen.m120ping FROM network_quality.autogen.ping GROUP BY srcIp,dstIp,time(120m) END

CREATE CONTINUOUS QUERY "1440m" ON network_quality BEGIN SELECT  mean(lost) as lost,mean(mdev) as mdev,mean(score) as score,mean(ttl) as ttl INTO network_quality.autogen.m1440ping FROM network_quality.autogen.ping GROUP BY srcIp,dstIp,time(1440m) END



sudo docker run --restart=always --name=kapacitor -p 9002:9002 -v /home/kapacitor/config.conf:/kapacitor/config.conf --network=bridge --link influxsrv -d  registry.cn-hangzhou.aliyuncs.com/zhenhai/kapacitor:latest ./kapacitord -config ./config.conf
docker run -d --name zookeeper -p 2181 -t wurstmeister/zookeeper
docker run --name kafka -v ~/kafka/server.properties:/opt/kafka_2.12-1.0.0/config/server.properties -e HOST_IP=localhost -e KAFKA_ADVERTISED_PORT=9092 -e KAFKA_BROKER_ID=1 -e ZK=zk -p 9092 --link zookeeper:zk -d registry.cn-hangzhou.aliyuncs.com/zhenhai/kafka:1.0.0
sudo docker run -p 8095:8095 -v ~/telegraf/telegraf.conf:/telegraf/telegraf.conf --network=bridge --link kafka --link influxsrv -d registry.cn-hangzhou.aliyuncs.com/zhenhai/telegraf:1.0.1 ./telegraf -v=0 --config ./telegraf.conf -log_dir=./log

sudo docker run --restart=always --name=realtime_metric -p 8000:8000 --network=bridge --link kafka -d registry.cn-hangzhou.aliyuncs.com/zhenhai/realtime_metric:1.0.0 ./main -v=9 -log_dir=./log
